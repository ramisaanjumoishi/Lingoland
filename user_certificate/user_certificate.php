...
$sql = "
    SELECT c.course_id, co.title, c.certificate_path
    FROM certificate_issue ci
    JOIN certificate c ON ci.certificate_id = c.certificate_id
    JOIN course co ON c.course_id = co.course_id
    WHERE ci.user_id = ? AND co.title LIKE ?
...
$stmt->execute();
$result = $stmt->get_result();
$certificates = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
$stmt->close();
...
body.dark .cert-card { background: linear-gradient(135deg,#20132d,#3a2951); border: 2px solid rgba(255,255,255,0.08); }
.cert-card:hover { transform: translateY(-8px) scale(1.03); box-shadow: 0 20px 40px rgba(106,76,147,0.25); border-color: #9b59b6; }
...
.cert-search-btn:hover {
    transform: translateY(-2px);
    box-shadow: 0px 8px 18px rgba(155, 89, 182, 0.35);
}
...
body.dark .search-bar input { background-color: #2c2c3c;
    border: 1px solid #555;
    color: #eee;}

body.dard .cert-card{background: linear-gradient(135deg,#20132d,#3a2951); border: 2px solid rgba(255,255,255,0.08); }
...
<div class="sidebar">
  ...
  <div class="menu">
     <a href="../user_dashboard/user-dashboard.php"><i class="fas fa-home"></i> <span>Home</span></a>
      ...
      <a href="../user_certificate/user_certificate.php"  class="active"><i class="fas fa-certificate"></i> <span>Certificates</span></a>
...
<!-- Main -->
<main>
    <h1>Complete Courses to Earn Certificates</h1>

    <!-- SEARCH BAR -->
    <div class="search-bar">
        <form method="GET">
            <input type="text" name="search" class = "search-input"placeholder="Search certificates..." 
                value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
            <button class="cert-search-btn" type="submit">Search</button>
        </form>
    </div>
            ...
            <?php foreach ($certificates as $c) { ?>
                <div class="cert-card stagger">
                    <div class="cert-img-wrapper">
                        <img src="<?php echo htmlspecialchars($c['certificate_path']); ?>" alt="Certificate">
                    </div>

                    <h3><?php echo htmlspecialchars($c['title']); ?></h3>

                    <button class="download-btn"
                            onclick="downloadCert('<?php echo $c['certificate_path']; ?>')">
                        Download
                    </button>
                </div>
            <?php } ?>
        <?php } else { ?>
            <p class="no-cert">No certificates earned yet.</p>
        <?php } ?>
    </div>
</main>
...
function downloadCert(path) {
    ...
    // Test if this is the problematic card
    if (!path || path === 'undefined') {
        alert("Error: No certificate path available for this course.");
        return;
    }
    
    window.location.href = "download_certificate.php?img=" + encodeURIComponent(path);
}

// Debug: Check all certificate paths on page load
document.addEventListener('DOMContentLoaded', function() {
    ...
    certCards.forEach((card, index) => {
        ...
        console.log(`Card ${index + 1}:`, {
            ...
            onclick: button ? button.getAttribute('onclick') : 'No onclick'
        });
    });
});
