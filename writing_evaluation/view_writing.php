...
$suggestions = json_decode($fb['suggestion_json'], true) ?? [];
...
<div class="sidebar">
  ...
  <div class="menu">
    ...
    <div class="sub" id="writingSub">
      ...
      <a href="../writing_evaluation/my_writing.php" class="active">My Writings</a>
    </div>
...
<main>
  ...
  <div class="we-container">
    <div class="we-left">
      <h1>Writing Evaluation Result</h1>
      <p class="we-sub">Asses your grammar, coherence & vocabulary and improve it.</p>
    </div>
    ...
    <div class="we-actions">
      ...
      <button id="btn_re_evaluate" class="we-btn" style="display:none;">Re-Evaluate It</button>
    </div>
  </div>
  ...
  <section class="we-panel">
   ...
   <div class="we-prompt">
    ...
    <div id="wePromptText" class="we-prompt-text">
       <?php echo htmlspecialchars($fb['writing_prompt']);?>
    </div>
    <div id="wePromptDifficulty" class="we-prompt-difficulty"><?php echo htmlspecialchars($fb['difficulty_level']); ?></div>
</div>
        <textarea id="weTextarea" class="disabled" disabled><?php echo htmlspecialchars($fb['submission_text']); ?></textarea>
...
$coherence = $fb['coherence_score'];
$vocab = $fb['vocabulary_score'];
$overall = $fb['overall_score'];
$summary = $fb['ai_feedback_summary'];
$suggestions = json_decode($fb['suggestion_json'], true) ?? [];
?>
      ...
      <div class="we-feedback" id="weFeedback">
        ...
        <div id="weSummary" class="we-summary"><?php echo nl2br(htmlspecialchars($summary)); ?></div>

        <h4 style="margin-top:18px">Issues found</h4>
        <div id="weIssues" class="we-issues">
           <?php foreach ($suggestions as $s): ?>
      <div class="we-issue-item">
    ...
    <aside class="we-right">
      <div class="we-scores">
        <div class="score-card">
          <div class="gauge" data-percent="<?php echo $grammar; ?>"id="g-grammar">
          ...
          <div class="gauge" data-percent="<?php echo $coherence; ?>" id="g-coherence">
            <svg viewBox="0 0 36 36">
              <path class="g-bg" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <path class="g-arc" stroke-dasharray="0,100" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <text x="18" y="20.35" class="g-text">0</text>
          ...
          <div class="gauge" data-percent="<?php echo $vocab; ?>"  id="g-vocab">
            <svg viewBox="0 0 36 36">
              <path class="g-bg" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <path class="g-arc" stroke-dasharray="0,100" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <text x="18" y="20.35" class="g-text">0</text>
          ...
          <div class="gauge overall" data-percent="<?php echo $overall; ?>" id="g-overall">
            <svg viewBox="0 0 36 36">
              <path class="g-bg" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <path class="g-arc" stroke-dasharray="0,100" d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831a15.9155 15.9155 0 1 1 0-31.831"/>
              <text x="18" y="20.35" class="g-text">0</text>
        ...
        <div style="margin-top:16px; text-align:center;">
          <button id="btn_edit" class="we-btn hollow">Improve Your Writing</button>
        </div>
      </div>
    </aside>
  </section>
  ...
  <script>
let original_text = document.getElementById("weTextarea").value;

document.getElementById("btn_edit").onclick = () => {
    document.getElementById("weTextarea").disabled = false;
    document.getElementById("weTextarea").classList.remove("disabled");

    document.getElementById("btn_edit").style.display = "none";
    document.getElementById("btn_re_evaluate").style.display = "inline-block";
};


document.getElementById("btn_re_evaluate").onclick = async () => {
    let new_text = document.getElementById("weTextarea").value;
    ...
    // Show loading state
    const btn = document.getElementById("btn_re_evaluate");
    const originalText = btn.textContent;
    btn.textContent = "Evaluating...";
    btn.disabled = true;

    try {
        console.log("Sending re-evaluation request...");
        
        let res = await fetch("writing_revision_save.php", {
            method: "POST",
        ...
        if (data.ok) {
            ...
            let message = "Revision evaluated successfully!";
            if (data.improvement > 0) {
                message += ` Improvement: +${data.improvement} points!`;
            } else if (data.improvement < 0) {
                message += ` Score change: ${data.improvement} points.`;
            } else {
                message += " Score remained the same.";
            }
            alert(message);
            
            // Update the UI with new scores
            updateScores(data.scores);
            updateFeedback(data.scores);
            
            // Update original_text variable to prevent multiple re-evaluations
        ...
        // Reset button state
        btn.textContent = "Re-Evaluate It";
        btn.disabled = false;
    }
};

// Function to update scores in gauges
function updateScores(scores) {
    console.log("Updating scores:", scores);
    
    // Update gauge data attributes
    document.getElementById('g-grammar').setAttribute('data-percent', scores.grammar_score);
    document.getElementById('g-coherence').setAttribute('data-percent', scores.coherence_score);
    document.getElementById('g-vocab').setAttribute('data-percent', scores.vocabulary_score);
    document.getElementById('g-overall').setAttribute('data-percent', scores.overall_score);
    
    // Re-animate gauges with new scores
    setTimeout(() => {
        animateAllGauges();
    }, 100);
}

// Function to update feedback and issues
function updateFeedback(scores) {
    console.log("Updating feedback:", scores);
    
    // Update summary
    const summaryEl = document.getElementById('weSummary');
    if (summaryEl && scores.feedback_summary) {
        summaryEl.innerHTML = scores.feedback_summary.replace(/\n/g, '<br>');
    }
    
    // Update issues
    const issuesEl = document.getElementById('weIssues');
    if (issuesEl && scores.suggestions && Array.isArray(scores.suggestions)) {
        issuesEl.innerHTML = '';
        if (scores.suggestions.length > 0) {
            scores.suggestions.forEach(suggestion => {
                const issueItem = document.createElement('div');
                issueItem.className = 'we-issue-item';
                issueItem.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${suggestion}`;
                issuesEl.appendChild(issueItem);
            });
        } else {
            // No issues found
            const noIssues = document.createElement('div');
            noIssues.className = 'we-issue-item';
            noIssues.innerHTML = `<i class="fas fa-check-circle" style="color: #5efc8d;"></i> Great! No issues found.`;
            issuesEl.appendChild(noIssues);
        }
    }
}

// Function to re-animate all gauges with current data-percent values
function animateAllGauges() {
    document.querySelectorAll('.gauge').forEach(gauge => {
        const percent = parseInt(gauge.getAttribute('data-percent')) || 0;
        animateGauge(gauge, percent);
    });
}

// Improved gauge animation function
function animateGauge(gaugeEl, percent) {
    ...
    if (!arc) {
        console.error('Gauge arc not found for element:', gaugeEl);
        return;
    }
    ...
    setTimeout(() => {
        ...
        const counter = setInterval(() => {
            ...
            if (start >= percent - 0.5) {
                clearInterval(counter);
                if (text) text.textContent = percent; // Ensure final value is exact
            }
        }, step);
    }, 60);
}
...
document.getElementById("weTextarea").addEventListener("input", () => {
    let text = document.getElementById("weTextarea").value.trim();
    let count = text.length ? text.split(/\s+/).length : 0;
    document.getElementById("weWordCount").innerText = count + " words";
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Animate initial gauges
    setTimeout(animateAllGauges, 500);
});
...
window.addEventListener('load', function() {
    function animateGauge(gaugeEl) {
        const percent = parseInt(gaugeEl.getAttribute('data-percent')) || 0;
        const arc = gaugeEl.querySelector('.g-arc');
        const text = gaugeEl.querySelector('.g-text');
        const dash = percent + ',100';
