...
if (!$result) {
    die("Error fetching badges: " . mysqli_error($conn));
}
...
body.dark .badge-card { background: linear-gradient(135deg,#20132d,#3a2951); border: 2px solid rgba(255,255,255,0.08); }
...
.badge-card {
  ...
  animation: badgeSlideIn .6s forwards ease;
}
...
.badge-card:hover {
  transform: translateX(6px);
  border-color: #b57aff;
  box-shadow: 0 16px 44px rgba(155, 89, 182, 0.35);
}
...
body.dark .badge-title { color: #e7d6ff; }
...
body.dark .badge-description { color: #bdb8ce; }
...
.btn-earned {
  ...
  color: #4a3300;
  box-shadow: 0 6px 18px rgba(255,210,111,0.35);
  align-self: center;
}
...
body.dark .btn-not-earned {
  background: #2c2c37;
  border-color: #444;
  color: #c9c9c9;
}

.btn-not-earned:hover {
  transform: translateY(-3px);
  background: #cfcfcf;
  box-shadow: 0 6px 18px rgba(0,0,0,0.12);
}

body.dark .btn-not-earned:hover {
  background: #3a3a47;
...
<div class="sidebar">
  ...
  <div class="menu">
    ...
    <a href="../user_badge/user_badge.php" class="active"><i class="fas fa-award"></i> Badges</a>
...
<div class="container fade-in">
    ...
    <div class="lb-header">
      <div>
        <h2>Unlock new Badges to level up your position in leaderboard 🏅</h2>
      </div>
            ...
            <div class="badge-grid">
                    <?php
                            ...
                            $user_score_query = "SELECT score FROM user WHERE user_id = ?";
                            $stmt = $conn->prepare($user_score_query);
                            $stmt->bind_param("i", $user_id);
                            $stmt->execute();
                            $user_score_res = $stmt->get_result();
                            $user_data = $user_score_res->fetch_assoc();
                            ...
                            while ($badge = mysqli_fetch_assoc($result)) {
                                ...
                                $threshold = $matches[0] ?? 0;

                                
                                $earned_query = "SELECT * FROM earned_by WHERE user_id = ? AND badge_id = ?";
                                $stmt = $conn->prepare($earned_query);
                                $stmt->bind_param("ii", $user_id, $badge_id);
                                $stmt->execute();
                                $earned_res = $stmt->get_result();
                                $already_earned = $earned_res->num_rows > 0;
                                $stmt->close();

                                
                                if ($user_score >= $threshold && !$already_earned) {
                                    $insert_query = "INSERT INTO earned_by (user_id, badge_id, earned_on) VALUES (?, ?, NOW())";
                                ...
                                $button_class = $already_earned ? "btn btn-earned" : "btn btn-not-earned";
                                $button_text = $already_earned ? "Earned" : "Not Earned";
                                ?>

                                <div class="badge-card">
    <img src="<?php echo $badge['icon_url']; ?>" class="badge-icon">

    <h5 class="badge-title"><?php echo htmlspecialchars($badge['title']); ?></h5>
