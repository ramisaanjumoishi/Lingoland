...
// --- DB Connection ---
$conn = mysqli_connect("localhost", "root", "", "lingoland_db");
if (!$conn) die("DB connection failed: " . mysqli_connect_error());

// --- Get profile_id ---
$profile_id = null;
$stmt = $conn->prepare("SELECT profile_id FROM user_profile WHERE user_id = ? LIMIT 1");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$res = $stmt->get_result();
if ($r = $res->fetch_assoc()) $profile_id = (int)$r['profile_id'];
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $word_usage = trim($_POST['word_usage'] ?? '');
    $tags_input = trim($_POST['tags'] ?? '');
    $tags_array = array_filter(array_map('trim', explode(',', $tags_input)));
    $tags_json = json_encode($tags_array);

    if ($word_usage && $meaning) {
        // get next card_id from view all_flashcards
        $q = $conn->query("SELECT MAX(card_id) AS last_id FROM all_flashcards");
        $row = $q->fetch_assoc();
        $next_id = ((int)$row['last_id']) + 1;

        $ins = $conn->prepare("INSERT INTO user_created_flashcard 
            (card_id, user_id, profile_id, meaning, word_usage, tags, popularity, created_at)
            VALUES (?, ?, ?, ?, ?, ?, 0, NOW())");
        $ins->bind_param("iiisss", $next_id, $user_id, $profile_id, $meaning, $word_usage, $tags_json);
        if ($ins->execute()) {
            $success = true;
        } else {
        ...
        $error = "Please fill in both word and meaning.";
    }
}

// --- Fetch theme for dark/light mode ---
$theme_id = 1;
$tq = $conn->prepare("SELECT theme_id FROM sets WHERE user_id=? ORDER BY set_on DESC LIMIT 1");
...
/* Main Form Area */
main { margin-left:240px; padding:100px 28px; min-height:100vh; text-align:center; }
h1 {
  font-size:24px;
  font-weight:700;
  background: linear-gradient(90deg, #b57aff, #ffd26f, #9b59b6);
...
body.dark .form-container {
  ...
  background-image: linear-gradient(var(--card-dark), var(--card-dark)),
                    linear-gradient(135deg, #b57aff, #ffd26f, #9b59b6);
...
/* === Flashcard Creation Form Styling === */
...
.form-container {
  ...
  background-image: linear-gradient(var(--card-light), var(--card-light)),
                    linear-gradient(135deg, #b57aff, #9b59b6, #ffd26f);
...
body.dark .form-container {
  ...
  background-image: linear-gradient(var(--card-dark), var(--card-dark)),
                    linear-gradient(135deg, #b57aff, #ffd26f, #9b59b6);
  box-shadow: 0 20px 60px rgba(181, 122, 255, 0.18);
  color: var(--text-dark);
}
...
.input-group textarea:focus {
  border-color: #b57aff;
  box-shadow: 0 0 10px rgba(181, 122, 255, 0.4);
}
...
.submit-btn {
  display: inline-block;
  background: linear-gradient(135deg, #6a4c93, #b57aff);
  color: #fff;
  border: none;
  border-radius: 999px;
  padding: 12px 32px;
...
.submit-btn:hover {
  background: linear-gradient(135deg, #b57aff, #ffd26f);
  box-shadow: 0 6px 18px rgba(181, 122, 255, 0.4);
  transform: translateY(-2px);
}
...
body.dark .input-group textarea:focus {
  border-color: #ffd26f;
  box-shadow: 0 0 10px rgba(255, 210, 111, 0.4);
}
...
.alert-success { background: rgba(76, 175, 80, 0.2); color: #2e7d32; }
.alert-error { background: rgba(244, 67, 54, 0.2); color: #b71c1c; }
...
<div class="sidebar">
  ...
  <div class="menu">
    ...
    <div class="sub" id="flashSub">
      ...
      <a href="../user_flashcards/create_flashcard.php" class="active">Add New</a>
...
<!-- Main Content -->
<main>
  <h1>Create Your Custom Flashcard ✨</h1>

  <div class="form-container stagger">
    <?php if (!empty($success)): ?>
      <div class="alert alert-success">✅ Flashcard created successfully!</div>
    ...
    <form method="POST">
      <div class="input-group">
        <input type="text" name="meaning" id="meaning" placeholder=" " required>
        <label for="meaning">Flashcard Name (Word)</label>
      </div>

      <div class="input-group">
        <textarea name="word_usage" id="word_usage" placeholder=" " rows="4" required></textarea>
        <label for="word_usage">Flashcard Meaning / Description</label>
      </div>
      ...
      <button type="submit" class="submit-btn">Create Flashcard</button>
    </form>
  </div>
</main>
<!-- Chatbot -->
